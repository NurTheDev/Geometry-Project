getCalculate = () => {
  const [triangleBase, triangleHeight] = getInput(
    "triangleBase",
    "triangleHeight"
  );
  if (isNaN(triangleBase) || isNaN(triangleHeight)) {
    ...
    const total = 0.5 * triangleBase * triangleHeight;
    ...
    p.innerHTML = `1. Triangle: ${(span.innerText = total)} cm<sup>2</sup>`;
    ...
    button.addEventListener("click", () => {
      ...
      p.innerHTML = `1. Triangle: ${(span.innerText = meters)} m<sup>2</sup>`;
    });
  }
};
...
triangle.addEventListener("mouseenter", () => {
  triangle.style.background = getRandomColor();
});
