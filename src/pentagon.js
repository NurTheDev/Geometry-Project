getCalculatePentagon = () => {
  const [pentagonPerimeter, pentagonBase] = getInput(
    "pentagonPerimeter",
    "pentagonBase"
  );
  if (isNaN(pentagonPerimeter) || isNaN(pentagonBase)) {
    ...
    const total = 0.5 * pentagonPerimeter * pentagonBase;
    ...
    p.innerHTML = `1. pentagon: ${(span.innerText = total)} cm<sup>2</sup>`;
    ...
    button.addEventListener("click", () => {
      ...
      p.innerHTML = `1. pentagon: ${(span.innerText = meters)} m<sup>2</sup>`;
    });
  }
};
...
pentagon.addEventListener("mouseenter", () => {
  pentagon.style.background = getRandomColor();
});
pentagon.addEventListener("mouseleave", () => {
  pentagon.style.background = "white";
});
