getCalculateParallelogram = () => {
  const [parallelogramBase, parallelogramHeight] = getInput(
    "parallelogramBase",
    "parallelogramHeight"
  );
  if (isNaN(parallelogramBase) || isNaN(parallelogramHeight)) {
    ...
    const total = parallelogramBase * parallelogramHeight;
    ...
    p.innerHTML = `3. Parallelogram: ${(span.innerText =
      total)} cm<sup>2</sup>`;
...
parallelogram.addEventListener("mouseenter", () => {
  parallelogram.style.background = getRandomColor();
});
parallelogram.addEventListener("mouseleave", () => {
  parallelogram.style.background = "white";
});
